...
public class GithubService {
    ...
    public List<CommitResult> getRepository(final String name) throws IOException {
        ...
        for (GHCommit commit : ghRepository.listCommits()) {
            try {
                final CommitResult commitResult = new CommitResult(commit.getCommitShortInfo().getMessage(), commit.getCommitDate().getTime(), commit.getSHA1(), commit.getParents().stream().map(GHCommit::getSHA1).collect(Collectors.toList()));
